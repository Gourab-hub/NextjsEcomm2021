                ...
                price = price + item.quantity * item.product.price
                  return(
                    <div className="container">
                      <div className="card qcard" style={{display:"flex",margin:"20px"}} key={item._id}>
                          <img src={item.product.mediaUrl} style={{width:"30%"}}/>
                          <div style={{marginLeft:"20px"}}>
                              {/* <h6>{item.product.name}</h6> */}
                              <span className="card-title ">{item.product.name}</span>
                              <h6 className="card-title ">{item.quantity} x  ₹ {item.product.price}</h6>
    ...
    const TotalPrice = ()=>{
        return(
            <div className="container" style={{display:"flex",justifyContent:"space-between"}}>
                ...
                billingAddress={true}
                zipCode={true}
                stripeKey="pk_test_51HylWvKweTzOB9zvrU00GBZ6XkriiMnPlMSOg72329xu9ga6FKPa1bZIsSHhhmEDmWZ5VPK1mUKhyNDQXfql4vRP00VxjZFtNP"
